...
const grads = (type, colArr1, colArr2, mode) => {
    ...
    let grad1 = `repeating-linear-gradient(45deg, ${stripe1(colArr1)}), repeating-linear-gradient(0deg, ${stripe1(colArr2)}), rgba(${mode},1)`   

    // ----------------------------------------------------------------------------------
        
    let grad2 = `conic-gradient( at 10% -10%, ${conic2(colArr1)} ), repeating-radial-gradient(circle at 10% 0%, ${curve(colArr1)} ), rgba(${mode},1)`

    // ----------------------------------------------------------------------------------
    
    let grad3 = ` repeating-linear-gradient(60deg, ${lin2}), repeating-linear-gradient(120deg, ${stripe1(colArr1)} ),  repeating-linear-gradient(240deg, ${stripe1(colArr2)} ), rgba(${mode},1)`

    // ----------------------------------------------------------------------------------

    let grad4 = `linear-gradient(45deg, ${strip45(colArr1)}), linear-gradient(135deg, ${strip45(colArr2)}), rgba(${mode},1)`
    
    // ----------------------------------------------------------------------------------
    
    let grad5 = `radial-gradient( ${radial(colArr1)} ), conic-gradient( ${conic(colArr2)} ), rgba(${mode},1)`
    
    // ----------------------------------------------------------------------------------

    let grad6 = `linear-gradient( ${lin2} ), linear-gradient(90deg, ${lin1(colArr1)} ), rgba(${mode},1)`
...
class CreateGradientBackground extends EventEmitter {

    constructor(type, colArr1, colArr2, mode){
        super()
        this.getGrad = grads(type, colArr1, colArr2, mode)
    }

    setGrad(type, colArr1, colArr2, mode){
        
        let missingValue = ''
        const findMissing  = (arr) => {
            
            for( let i = 0; i < arr.length; i++){
                let one = validator(arr[i])
                
                if( one.valid === false) {
                    missingValue += `${one.message}:  ${arr[i]} `
                } else{
                    missingValue = missingValue
                }
            }
        }
        let missing1 = findMissing(colArr1) 
        
        if(missingValue !== '') return this.getGrad = missingValue

        let missing2 = findMissing(colArr2) 
        if(missingValue !== '') return this.getGrad = missingValue
